...
class LayoutMultiplierTest {
    ...
    void compute() throws PlacementNotAllowedException, InvalidCoordinatesException, NoAdjacentCardException {
        ...
        assertEquals(0, LayoutMultiplier.OM_090.compute(playArea));
        ...
        assertEquals(1, LayoutMultiplier.OM_090.compute(playArea));

        // Changing Seed needed
        assertEquals(0, LayoutMultiplier.OM_088.compute(playArea));
        ...
        assertEquals(2, LayoutMultiplier.OM_090.compute(playArea));
        ...
        assertEquals(3, LayoutMultiplier.OM_090.compute(playArea));

        // Testing 2 Fungi Card in position : [0,0], [1,0] and a Plant Card in position [2,1]
        // With Objective number: 91
        playArea = new PlayArea();
        // Testing empty PlayArea
        assertEquals(0, LayoutMultiplier.OM_091.compute(playArea));
        ...
        assertEquals(1, LayoutMultiplier.OM_091.compute(playArea));

        playArea.playSide(ResourceBackSide.RBS_012, new Tuple(2,0));
        // Testing two conditions right
        assertEquals(2, LayoutMultiplier.OM_091.compute(playArea));
